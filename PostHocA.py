...
def make_cownames(id_file="ids_full_SPAN.csv"):
...
def add_final_reports(genos,final_rep="final_reports/*.txt"):
    fis=glob.glob(final_rep)
    for fi in fis:
    ...
    for lin in fil[10:]:
        lii=lin.split("\t")
        try:
          if lii[0] in snpnames and float(lii[-1])>0.7:
...
cownames['Mostrenca 7966']

# <codecell>

genos=create_genos(raw_data_folder="RawData_PNAS/*_100.txt")

# <codecell>

print("ok, did that at least")

# <codecell>

print(len(genos['888000018']))
...
#WHY YOU NO WORKIING!! - not writing ANYTHING TO FILES
